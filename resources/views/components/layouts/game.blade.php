...
<body @class(['game-body', 'game-body--nav' => $showNav])>
  ...
  @endif
  <main>{{ $slot }}</main>
  <audio id="timerWarnSound" preload="auto" playsinline src="{{ asset('audio/timer-warn.wav') }}"></audio>
  <audio id="timerEndSound" preload="auto" playsinline src="{{ asset('audio/timer-end.wav') }}"></audio>
  <script>
    window.SwalifAudio = {
      ctx: null,
      getCtx: function() {
