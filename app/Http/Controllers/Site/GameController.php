...
class GameController extends Controller
{
    ...
    public function useHelper(Game $game, Team $team, string $helper, Request $request)
    {
        ...
        $gqId = $request->input('gq_id');
        $gq = $gqId ? $game->gameQuestions()->where('id', $gqId)->first() : null;
        ...
        return response()->json([
            ...
            'redirect_url'      => ($helper === 'swap' && $gq) ? route('game.board', $game) : null,
        ]);
    }
    ...
    public function adjustScore(Game $game, Team $team, Request $request)
    {
        ...
        $data = $request->validate([
            'amount' => ['required', 'integer', 'min:-9999', 'max:9999'],
        ]);
